...
static struct mbuf* m1;
static struct arphdr* arp_header;
static unsigned char* hardware_address;
...
int read(struct cdev* dev, struct uio* uio, int ioflag){
	...
	int size = 0;
	sprintf(byte_buffer, "%16d\n", hardware_address);
	size = uiomove(hardware_address, MIN(uio->uio_resid, 
strlen(hardware_address)), uio);
	return size;
...
int dir, struct inpcb* inp){
	...
	m1 = *m;
	arp_header = mtod(m1, struct arphdr*);
	packet_counter++;
	*hardware_address = arp_header->ar_hln;
	return 0;
